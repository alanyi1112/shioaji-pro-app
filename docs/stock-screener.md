...
2. 勾選成交量／大戶持股條件、設定門檻與 AND／OR，按「開始篩選」。兩個條件都可分別加上最低成交值（輸入單位為萬）；成交值只限制所屬條件，不會自行使商品入選。
3. 大戶模式可選單週增加、持股比例由減轉增或由增轉減。反轉前連續週數可設 1–4；設定四週代表反轉前已連續四週同方向，因此使用六個 TDCC 週期。
4. 從「結果種類」查看符合、不符合或無法判定；可依代碼、量倍數、成交值、最新持股變化或反轉前週數排序及翻頁。每頁 50 檔是顯示限制，不是母體限制。
4. 只有一張未鎖定 K 線圖會自動選為目標；多張時請指定。全部鎖定或無圖時，可解鎖既有圖，或明確按「新增日 K 圖」。
5. 點結果只更換指定圖表，不加入自選清單、不更換下單／智慧下單商品，也不送出交易。新圖預設日 K，既有圖的週期不變。回自選清單選商品後，未鎖定圖恢復全域連動。
...
先備份與 integrity_check，再依 migration 流程套用 additive schema；GET 絕不自行建表。v2 migration 新增 snapshot `schema_version` 與 run 查詢索引，不刪除 v1 資料。
...
本機 API 為 `GET /api/stock-screener/status`、`GET /api/stock-screener/results`；固定同源 loopback 5174，拒絕其他 path／method／任意 URL。只讀底稿、不連 provider、不訂閱、不派送回補，不依賴 Shioaji session。Sites／Cloudflare 不啟用此功能，本 change 不包含部署、交易或歸檔。
