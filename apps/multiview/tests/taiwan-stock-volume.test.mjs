...
test("production wiring 使用有界日 Kbars、generation guard、強制模式 fail-closed 與完整 fallback payload", () => {
  ...
  assert.match(coordinatorSource, /const representation = needsMinuteHistory \? "minute" : "daily";/);
  assert.match(coordinatorSource, /representation === "daily"\s*\? dailyFromKbars\(payload\)\s*: sessionFromKbars\(payload\)/);
  ...
  assert.match(appSource, /function renderDailyKlines\(snapshot = latestRealtimeSnapshot\) \{\s*if \(!dailyKlineAccumulator \|\| !candleSeries\) return;/);
  assert.match(appSource, /const quote = snapshot\s*\? realtimeQuote\(snapshot, realtimeDisplayState\)\s*: shioajiHistoryQuote\(candles, realtimeDisplayState\);/);
  assert.match(appSource, /hasShioajiKbars[\s\S]*?保留最近 Shioaji Kbars[\s\S]*?!hasShioajiKbars && \(isMinuteKlineInterval/s);
  ...
  assert.match(appSource, /applyPayload\(payload, \{ preserveVisibleLogicalRange, oldCandleCount \}\);/);
  assert.match(appSource, /if \(realtimeDisplayState === "live"\) \{\s*status\.textContent = `\$\{symbolSelect\.value\} \/ 日 已載入 Shioaji`;\s*status\.classList\.remove\("is-visible"\);\s*\} else if \(realtimeDisplayState === "degraded"\) \{\s*status\.textContent = snapshot\s*\? "即時連線不穩，顯示最後可用行情"\s*: "已載入 Shioaji Kbars，等待即時 snapshot";/s);
