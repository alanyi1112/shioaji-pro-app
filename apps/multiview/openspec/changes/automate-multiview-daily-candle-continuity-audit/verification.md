...
- D1-only acceptance 首次正式產出揭露讀取邊界 off-by-one：只讀 160 rows 時，最新 session 正規化後可能剩 159 根。snapshot 改為固定多讀 5 根 buffer，再由既有 payload 路徑裁成 160／320；server 端同時先驗證產出 candle count，不足時 fail closed 進入官方回補，不補造 candle。
