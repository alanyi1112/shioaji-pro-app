...
describe('Fibonacci overlay model', () => {
    it('第一張七級色帶、第二張單色且沒有第二組 band', () => {
        ...
        expect(model.bands).toHaveLength(6);
        expect(new Set(model.bands.map((band) => band.color)).size).toBe(6);
    ...
    it('畫面空間不足時標籤移入線內且不越過價格軸安全邊界', () => {
        ...
        expect(model.lines.filter((line) => line.kind === 'level')).toHaveLength(7);
