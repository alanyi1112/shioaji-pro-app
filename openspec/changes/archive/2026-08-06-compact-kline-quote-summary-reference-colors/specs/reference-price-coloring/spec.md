...
K 棒價量 readout 的開、高、低、收／最新欄位 MUST 在可取得該 candle 所屬目前交易日／交易時段的可靠 reference 時逐欄判色；時間與量 MUST 維持中性色。系統 MUST 保持 K 棒時間區間與價量 readout 位於顯示數值區最上方的既有規則。
...
#### Scenario: 歷史股票 K 棒缺少官方 reference
- **WHEN** 游標位於台灣今日以前的 candle，且資料模型沒有該日官方 reference
- **THEN** open、high、low、close MUST 全部使用 flat 樣式
- **AND** 系統 MUST NOT 使用今天 reference 或前一根 close 冒充該日官方 reference

#### Scenario: 期貨或選擇權無法判定交易時段
- **WHEN** FUT 或 OPT candle 不是可證明仍 forming 的最新 candle，或無法證明目前 reference 適用於該交易時段
- **THEN** readout 價位 MUST 使用 flat 樣式

#### Scenario: 游標從當日移到歷史 K 棒
- **WHEN** crosshair 從可判色的當日 candle 移至無可靠 reference 的歷史 candle
- **THEN** readout MUST 在同一個有界更新週期內改為 flat
