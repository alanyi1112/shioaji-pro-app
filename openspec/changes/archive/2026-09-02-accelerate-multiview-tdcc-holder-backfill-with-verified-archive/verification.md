...
- [x] Archive bootstrap 版本 v223 後，DB-only warm path 修正版亦已推送 Sites source repo exact SHA `731aa7bfb2c9ddce61668ccc3c7e92dbc0eed85b`；Sites v224、deployment `appgdep_6a97a880146c8191907ed472a9409ef9`、environment revision 45 部署成功，owner-only custom access policy 維持 1 位 owner、0 groups。
...
- [x] v224 專用 release tree 完整 `npm test` 為 698／698 通過；lint、Cloudflare budget、build 與 `git diff --check` 通過。主專案新增 DB-only warm path regression test 後，聚焦 chip service tests 33／33 通過；既有 archive tests、MultiView lint 與 typecheck 亦通過。
...
- [x] Owner 身分驗收先確認 8103 不在個人清單，再從「我的清單」加入 8103.TW 瀚荃；重整後 URL 與商品選擇器均為 8103.TW，沒有使用 machine bypass 或 `SAMPLE` 畫面。大戶最新為 2026-08-28、32.16%、週變化 +7.95%、+6,231.5 張、11 人、人數變化 +4；散戶為 23.11%、週變化 -2.13%、-1,675.7 張、18,389 人、人數變化 -84。
- [x] 8103 大戶與散戶主 canvas 均可見，CSS 尺寸各為 646×70、bitmap 各為 1,292×140；console warning／error 為零。頁面只送出 Sites 同源 `/api/taiwan-stock-chip` request，未送出 TDCC open data、archive GitHub／raw、FinMind、TWSE 或 TPEX 外部 request。
- [x] Owner 身分另加入代表性 `.TWO` 商品 6488.TWO 環球晶並重整；大戶副圖可見，最新為 2026-08-28、70.46%、週變化 -0.95%、-4,509.7 張、46 人、人數變化 -2，主 canvas CSS 646×36、bitmap 1,292×72，console warning／error 為零，network 僅有一筆同源 shareholder-distribution API request。4768.TWO 晶呈科技在修正前的 UI 驗收遇到 K 線 API transient timeout，因此未拿它冒充通過的 `.TWO` canvas 證據；其 protected shareholder-distribution API 完整性仍由前述 8.3a 證據涵蓋。
- [x] Owner 驗收同時發現既有多期 D1 rows 的互動 GET 仍可能進入 `d1_refreshed` 並等待 provider；v224 已改為 shareholder-distribution 有至少兩期 verified D1 rows 時直接走 DB-only warm path。新增 regression test 證明 provider calls 為 0、`cache.mode=d1_hit` 且兩期資料 available；部署後 6488.TWO 不再因 shareholder provider 等待而逾時，8103／6488 的瀏覽器 network 亦沒有任何 archive／official provider request。另以 fresh protected no-cache request 實測 v224 的 8103 回應為 HTTP 200、`cache.mode=d1_hit`、22 期、最新 2026-08-28，archive receipt 為 target／processed 18／18、remaining／failed／overdue 0／0／0、`complete=true`；未採用先前已開啟 API 文件分頁的舊快取回應。
...
- Cloudflare 額度恢復後的 exact SHA、18 期 receipts、all-zero health、official 51 週、代表 API、DB-only warm path、DOM／canvas／console／network 與非預期 request 核對，已完整移交 active change `complete-cloudflare-tdcc-verified-archive-acceptance-after-quota-reset`。原 tasks 8.2、8.3c、8.4b、9.1～9.4、9.5b 的勾選只代表移交完成，不代表 Cloudflare 實際驗收已通過。
- Sites owner 身分的 8103／`.TW`／`.TWO` DOM、可見 canvas、週變化、console／network 與新增商品 DB-only warm path 已於 v224 完成；Cloudflare 額度恢復前，後續 change 的實作 tasks 全部保持未勾選，且不得以 Sites 或本機證據替代。
